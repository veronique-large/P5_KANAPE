...
if (DonneesLocalStorage){//je cree une condition si il y a des donnees dans le sessionStorage
    ...
    */
    ...
    // son innerHTML la variable Y, fermer la function et la lancer
        then(function(somme){
    let total = number(DonneesLocalStorage.value);
    DonneesLocalStorage.value = document.getElementById(totalQuantity).value * document.getElementById(totalPrice);

    for(let totaux of DonneesLocalStorage.value ){
         DonneesLocalStorage.value += product.quantity;       
    }
    /*let articles = document.getElementById('totalQuantity');
    articles.innerHTML = product.quantity;*/
